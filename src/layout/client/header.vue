<template>
    <div class="bg-black w-full flex flex-col fixed top-0">
            ...
            <div class="flex-1 h-full flex items-center justify-end flex-row gap-5">
                ...
                <div v-if="isLogin"
                    ...
                    <div
                        ...
                        <div class="p-5 w-full uppercase font-bold text-xl hover:bg-gray-300 flex flex-row items-center gap-5"
                            @click="handleLogout">
                            <svg class="w-6 h-6 text-black" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                                width="24" height="24" fill="none" viewBox="0 0 24 24">
                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
        ...
        <div class="h-12 w-full bg-red-300 flex flex-row">
            <div class="h-full flex-1 flex flex-row items-center justify-start uppercase font-bold text-xl">
                <a class="h-full flex items-center p-5" href="/">home</a>
                <div class="hover:bg-gray-300 h-full flex items-center p-5 relative group">
                    <p class="hover:cursor-context-menu">category</p>
                    <div class="w-60 h-fit bg-white absolute top-12 left-0 flex-col hidden group-hover:flex">
                ...
                <a class="hover:bg-gray-300 h-full flex items-center p-5" href="/men">men</a>
                <a class="hover:bg-gray-300 h-full flex items-center p-5" href="/women">women</a>
            </div>
            <div class="h-full flex-1  flex flex-row items-center justify-end uppercase font-bold text-xl pr-5 gap-5">
                <a href="">
                ...
                <div class="relative">
                    <input @input="onInput" v-model="keyword" class="rounded-xl h-10 pl-5" type="text"
                        placeholder="Search" />

                    <div :class="'absolute flex flex-col gap-2 left-0 overflow-auto z-10 w-full mt-2 rounded-md bg-white shadow-2xl transition-all duration-200 ease-out '
                        + (isShow ? 'max-h-[500px] p-2' : 'max-h-0 p-0')">
                        <div v-if="resultsSearch.length != 0 && isShow" v-for="item in resultsSearch"
                            class="flex flex-row gap-2" :key="item.id">
                            <div @click="goToDetailPage(item.id)"
                            ...
                            <div class="text-sm flex-1 relative">
                                <p @click="goToDetailPage(item.id)" class="line-clamp-2 font-bold cursor-pointer">
                                    {{ item.name }}
                                </p>
                                <p class="font-medium text-red-400">{{ fCurrency(item.price) }}</p>

                                <div class="absolute bottom-0 flex justify-between gap-2">
                                    <button @click="goToBuyPage(item)"
                                        class="p-2 bg-yellow-300 rounded-sm hover:bg-yellow-400">Buy now</button>
                                    <button @click="addToCart(item)"
                                        class="p-2 bg-yellow-300 rounded-sm hover:bg-yellow-400">Add to
                                        cart</button>
                                </div>
                            </div>
                        </div>
                        <div v-else-if="resultsSearch.length == 0 && isShow" class="text-center">
                            not found
                        </div>
                    </div>
                </div>
