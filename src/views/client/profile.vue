<template>
  <div class="w-full h-fit flex justify-center mt-32 p-5">
    <div
      class="w-[80vw] h-full flex justify-center items-start flex-row border border-black rounded-3xl"
    >
      <div class="flex-1 h-full">
        <div class="h-full w-full p-20 flex flex-col gap-5 items-center">
          <div
            class="bg-white w-80 h-80 rounded-full overflow-hidden border-8 border-blue-500"
          >
      ...
      <div class="flex-1 h-full">
        <div
          class="h-full w-full p-20 flex flex-col gap-5 items-start font-bold text-2xl"
        >
          <div class="flex flex-row w-full">
            <p class="flex flex-1">Account</p>
            <input
              class="flex flex-1 border border-black rounded-2xl pl-5"
              type="text"
              :value="user.account"
              disabled
            />
          ...
          <div class="flex flex-col w-full gap-5">
            <p>Address</p>
            <input
              class="border border-black rounded-2xl pl-5 h-20 font-normal text-base"
              type="text"
              v-model="user.address"
            />
          </div>
          <button
            class="bg-orange-500 h-10 w-20 rounded-full"
            type="button"
            @click="saveProfile"
          >
            Save
  ...
  methods: {
    ...
    async saveProfile() {
      try {
        ...
        toast.toastrError("Failed to save profile.");
      }
    },
  },
  mounted() {
